...
### Round-35 adversarial-refutation findings (kk04/kk07/kk08/kk09/kk12) and their fixes

A round-35 refuter (again briefed to keep pressure-testing prior rounds' own
fixes) left 13 new probes (kk01-kk13, 8 already passing/honest); this
section covers the 5 that were failing.

| # | Finding | Fix | Program(s) |
|---|---|---|---|
| 1 | **`LINAGE IS <n> LINES WITH FOOTING AT 0` was silently coerced to "no FOOTING clause at all"** - the FD-registry-building loop in `generator/scala-generator.js` derived `footingLines` for the EOP-threshold formula by nulling out any value `<= 0`, then reused that ALREADY-nulled value for round-34's own invalid-FOOTING check (`footingLines > pageSize`) - so `FOOTING AT 0` (equally invalid, real cobc aborts at OPEN time with `LINAGE values invalid (status = 57)`) never tripped the check at all | The invalid-FOOTING check now inspects the RAW parsed `f.linageFootingLines` directly (an integer `<= 0` is invalid, the same bucket as `footingLines > pageSize`), instead of the already-nulled derived value. Verified against installed GnuCOBOL and scala-cli (kk04): the generated Scala now aborts at OPEN with zero output, matching cobc; confirmed zero regression on jj01/jj02 (valid FOOTING) and hh01 (bare LINAGE, no FOOTING at all) | kk04 |
| 2 | **BY CONTENT parameters to a RECURSIVE program's `entry()` got a caller-side no-op setter (correct - the caller's own copy must never be mutated by the callee), but that ALSO discarded a write made WITHIN the callee's own activation** - a later read in the SAME call never saw the mutation, so `SUBTRACT 1 FROM LK-DEPTH` had no visible effect at all, causing genuine unbounded recursion (`java.lang.StackOverflowError`). **First attempt at this fix (reverted after independent verification found a real regression):** changed `generateRecursiveEntryMethod` (`generator/scala-generator.js`) to give EVERY LINKAGE leaf - BY CONTENT and BY REFERENCE alike - a local var cached once from `_get()` at `entry()`. This broke BY REFERENCE: this codebase's WORKING-STORAGE model for RECURSIVE programs treats WORKING-STORAGE as SHARED, non-reentrant storage across the whole recursion call chain (unchanged, pre-existing), so a BY-REFERENCE-aliased LINKAGE parameter's correct, cobc-verified behavior requires every read to be LIVE - caching it once at entry made an OUTER activation's later read stop reflecting a DEEPER call's mutation of the same shared storage, producing an "off by one RECURSIVE depth" regression that broke three already-promoted, previously-passing oracle programs (`j10-recursive-call-ws.cbl`, `dd02-declaratives-recursive.cbl`, `k01-recursive-group-linkage.cbl`) and their own regression-guard unit tests (`tests/round21-fixes.test.js` finding 2, `tests/round22-fixes.test.js` finding 1, `tests/round23-fixes.test.js` finding 3, `tests/round28-fixes.test.js` finding 1) | **Corrected fix:** `generateRecursiveEntryMethod` is reverted to its exact pre-round-35 form (plain `def <leaf>: T = _get${i}()` / `def <leaf>_=(v: T): Unit = _set${i}(v)`, mode-agnostic, for every leaf - no per-activation local-var caching at `entry()` itself). The actual fix is at the CALL SITE instead, in `generateCall`'s `target.recursive` branch (`generator/expression-gen.js`): a BY CONTENT/VALUE argument (in the plain-ref-var branch, the named-group per-leaf branch, and the zero-default fallback branch alike) now gets its own isolated, call-site-scoped local snapshot var (`var _call<i>Snapshot<j>: T = <initial value>`, declared just before the `entry(...)` call), with the getter/setter closures passed to `entry(...)` operating on THAT local var instead of a no-op setter over the caller's own storage - giving BY CONTENT/VALUE a real, working, per-call-site-local mutable copy (a write is visible to a later read within the SAME callee activation) without ever writing back to the caller. A BY REFERENCE plain-var/named-group operand is completely unaffected - still a live getter/setter pair aliasing the caller's own variable directly, exactly as this finding's very first (pre-regression) intent for that mode. Verified against installed GnuCOBOL and scala-cli: kk07 no longer StackOverflows and now byte-matches its oracle (`DEPTH=3`/`DEPTH=2`/`DEPTH=1`, terminating normally, `AFTER CONTENT=100 REF=130`); j10/dd02/k01 all byte-match their oracles again (the regression is fixed); kk13 (the non-recursive-callee regression guard for this same finding) is unaffected, since it never reaches `generateRecursiveEntryMethod`/the `target.recursive` branch at all | kk07 (regression-fixed: j10, dd02, k01; unaffected: kk13) |
| 3 | **An OCCURS table REDEFINED by a DIFFERENTLY-SHAPED OCCURS table (different element count/width, same total byte width) was treated as a plain SCALAR alias (`def wsTableB: String = wsTableA`) by `redefinesAccessorLines`'s elementary-alias branch**, while the separate table-subscript codegen path still emitted Vector-indexed `wsTableB(0)` syntax against it - a hard type mismatch at Scala compile time | Added a new `occursElementaryOverOccursElementaryRedefinesLines` helper (`generator/scala-generator.js`) that builds a real synthetic flat-character view over the target's own `Vector[String]` (`targetCamel.mkString`), resliced into the redefining item's own element boundaries - a write through either view is visible through the other, matching COBOL REDEFINES storage-sharing semantics. Falls back to an honest, visible `??? `/no-op decline marker for any shape this model can't represent (mismatched total byte width, non-`PIC X` elements, etc. - not exercised by any corpus program, since such COBOL would be invalid). Verified against installed GnuCOBOL and scala-cli (kk08): `B1=[AABB]`/`A1=[WX]`, matching cobc's own character-reslicing exactly | kk08 |
| 4 | **`SEARCH ALL` over an OCCURS-bearing GROUP child nested under a REDEFINES (a table-of-groups reached via REDEFINES) crashed at Scala COMPILE time with `Not found: wsIdx`** - the INDEXED BY var was never declared anywhere, since this shape falls outside `buildFieldRegistry`'s own outer `walk()` (which never reaches an item hidden inside a REDEFINES's own children this way); the pre-fix `characterSlicedGroupRedefinesLines` fell through to its ordinary leaf-processing code, silently producing a zero-width slice and no `tableRegistry` entry, so `SEARCH ALL` degraded to its own honest decline comment for the TABLE - but the bare `SET <idx> TO ...` before it (which does not consult `tableRegistry`) compiled to a raw, undeclared identifier: a hard Scala compile failure, not an honest decline | `characterSlicedGroupRedefinesLines` now detects this exact shape (an OCCURS-bearing GROUP child nested under a REDEFINES) and declares the table's own `INDEXED BY` name(s) as real, working `Int` vars (exactly like `buildFieldRegistry`'s own outer declaration), while the table itself gets a visible, compiling `??? `/no-op stub (`TODO REDEFINES: unsupported table-of-groups-under-REDEFINES shape`) instead of a silently mis-registered, zero-width leaf with an undeclared index. Verified against installed GnuCOBOL and scala-cli (kk09): the generated Scala now compiles cleanly and runs to completion (an honest `t.todo(...)` decline for the table itself, not a compile crash) | kk09 |
| 5 | **`MOVE CORRESPONDING` plus a qualified REDEFINES field name collision (`A-VIEW-SUB OF GROUP-A`, where GROUP-A's REDEFINES-nested `A-VIEW-SUB` collides with GROUP-B's own plain `A-VIEW-SUB`) crashed at Scala COMPILE time with `Not found: aViewSub`** - the REDEFINES target-lookup path (round-3 finding 6's own reasoning) never anticipated an AMBIGUOUS elementary target (whose bare registry entry is deliberately skipped once its name collides), and the REDEFINES-nested child's own registration never participated in the ambiguous-name disambiguation pass ordinary (non-REDEFINES) leaves already use at all - leaving every one of this REDEFINES item's own children completely undeclared. A companion gap: a GROUP redefining a NUMERIC target whose own child is alphanumeric (not a numeric digit-slice) hit the same digit-arithmetic loop meant only for numeric children | `redefinesAccessorLines`'s target lookup now also tries the qualified registry (`NAME::ANCESTOR`) when the bare lookup misses; every leaf `redefinesAccessorLines`/`characterSlicedGroupRedefinesLines` declare now goes through the same ambiguous-name-aware registration (bare name only when unambiguous, always a qualified entry) ordinary leaves already use. The companion gap is fixed via a synthetic flat-character view over the target's own DISPLAY digit text (`CobolFmt.digitsOf`/`numval`), for the narrow unambiguous shape this can model exactly (unsigned, whole-number target). Verified against installed GnuCOBOL and scala-cli (kk12): `A-VIEW-SUB=[123]` (A-FIELD1's own digit bytes), `MOVE CORRESPONDING` correctly leaves GROUP-B's own `A-VIEW-SUB` untouched, matching cobc exactly | kk12 |

See `tests/round35-fixes.test.js` for focused, toolchain-independent unit
tests of all 5 findings above (finding 2's tests were updated in place to
match the corrected fix once the first attempt's regression was found and
reverted - see the finding 2 write-up above), including regression coverage
confirming kk13 (non-recursive callee) is unaffected by finding 2's fix,
and jj01/jj02/hh01's own valid-FOOTING/no-FOOTING shapes are unaffected by
finding 1's fix.

Net effect on the whole-suite `todo` count: all 5 findings are genuine
crash-to-fix or silent-wrong-output-to-fix corrections; finding 2's fix
converts kk07 from a genuine `java.lang.StackOverflowError` to a byte-exact
oracle match, with zero regression on the three previously-passing
recursive-LINKAGE oracle programs its own first (reverted) attempt had
broken. Overall dishonest-finding count for round 35: 5 (kk04, kk07, kk08,
kk09, kk12) - with finding 2 requiring one additional corrective iteration
...
### Round-38 adversarial-refutation findings (nn01/nn02, nn03/nn04, nn07, nn09, nn13) and their fixes

A round-38 refuter left 15 new probes (nn01-nn15); 9 were already passing/
honest at hand-off (nn05, nn06, nn08, nn10, nn11, nn12, nn14, nn15, and
nn15 specifically built as nn03's own non-recursive control case). The
remaining 6 probes were dishonest, across 5 distinct root causes: nn02
(restated in combination by nn01) is a Scala COMPILE crash in the REDEFINES
registry; nn03/nn04 are a silently-lost writeback in RECURSIVE CALL BY
REFERENCE codegen; nn07 is a runtime crash plus several wrong FILE STATUS
codes; nn09 is silently wrong PICTURE P-scaling; nn13 is a silently wrong
multi-target COMPUTE ON SIZE ERROR. All 5 are now fixed.

| # | Finding | Fix | Program(s) |
|---|---|---|---|
| 1 | **A NUMERIC elementary item REDEFINES-ing a GROUP (`WS-NUM REDEFINES WS-GROUP PIC 9(4)`) is a documented, intentional decline in `elementaryOverGroupRedefinesLines` (`generator/scala-generator.js`, the `itemBaseType !== 'String'` branch, which stubs `def wsNum: Int = ???`) - but that branch never added a registry entry for `WS-NUM` at all. A later `MOVE WS-NUM TO <alphanumeric-target>` then found `sourceInfo == null` in `renderVariableMoveSource` (`generator/expression-gen.js`), skipped the intended numeric-to-digit-text conversion, and emitted `CobolFmt.fitLeft(wsNum, width)` - passing an `Int` where `fitLeft` expects a `String`, a hard `Found: Int, Required: String` Scala COMPILE error, not the intended honest decline** | For the narrow, unambiguous shape this CAN actually be modeled exactly - an UNSIGNED, whole-number (no decimal places) numeric item over a target whose children are all plain digit-text leaves - `elementaryOverGroupRedefinesLines` now builds a REAL accessor reusing the exact same ops-based flat-character view it already builds for the String case (`CobolFmt.numval`/`CobolFmt.digitsOf` round-tripping the target's own digit-text children), registering it with `dataType: 'numeric'` so `renderVariableMoveSource` finds a real entry and routes through its ordinary numeric-to-alphanumeric conversion instead of crashing. Any OTHER numeric shape (signed, has decimal places, or an unmodelable target) still falls back to the pre-existing honest `???`/no-op stub pair, but now ALSO registers a real (if honestly-degraded) placeholder registry entry for it - matching round-17's `refModNumericPlaceholder` precedent of substituting a concrete, honestly-wrong stand-in rather than leaving no registry entry at all - so `sourceInfo` is never null for this item regardless of shape. Verified against installed GnuCOBOL and scala-cli: nn02 (`TARGET=[1234      ]`) and nn01 (the fuller LINAGE/RECURSIVE/REDEFINES combination probe, full `NOTEOP`/`EOP DEPTH=.../DATE=...` sequence through `MAIN AFTER DEPTH=00`) both byte-match their oracles exactly - not just an honest decline, the actual correct value | nn02 (restated by nn01) |
| 2 | **`CALL ... USING BY REFERENCE <subscripted-or-ref-modified-operand>` into a RECURSIVE program silently lost the callee's writeback** - `generateCall`'s `target.recursive` branch (`generator/expression-gen.js`) only recognized a "plain, unsubscripted, non-ref-mod" operand (`isPlainRefVar`) for true getter/setter-closure aliasing back to the caller's real storage (per round-21/35/36's own comments in this function); a subscripted (`WS-TABLE(WS-IDX)`) or ref-modified (`WS-STR(3:4)`) BY REFERENCE operand instead silently fell into the BY-CONTENT/VALUE call-site-snapshot convention (round-35/36's own mechanism), which never writes back to the caller after the call returns - a callee's mutation through such a parameter was completely lost from the caller's perspective (nn03: expected `AFTER ITEM2=320`, engine gave unchanged `020`; nn04: expected `AFTER STR=[ABZZZZGHIJ]`, engine gave unchanged `[ABCDEFGHIJ]`). nn15 (the same subscripted-BY-REFERENCE shape into a NON-recursive callee) already worked correctly, confirming the bug was narrowly specific to the RECURSIVE call path | `generateCall`'s `target.recursive` branch now recognizes two more shapes (`isSubscriptedRefVar`/`isRefModRefVar`), each building a REAL live getter/setter closure pair when `mode === 'REFERENCE'`: the subscripted case reuses `convertIdentifier`/`renderAssignment` verbatim (the exact same subscript-read/subscript-write convention this generator's ordinary non-recursive CALL writeback path already uses - see `scalar-subscripted`/`renderWriteback`), and the ref-mod case (only for a plain alphanumeric/String-typed base field - the narrow, unambiguous shape this can model exactly, matching round-35 finding 5/kk12's own precedent) builds a direct character-splice read (`wsStr.substring(_s, _s + _l)`) and write (`wsStr = wsStr.substring(0, _s) + v + wsStr.substring(_s + _l, wsStr.length)`). A BY CONTENT/VALUE operand of either shape is completely untouched - both new checks are gated on `mode === 'REFERENCE'` and simply fall through to the existing snapshot mechanism otherwise. Verified against installed GnuCOBOL and scala-cli: nn03 and nn04 both byte-match their oracles exactly; nn15 (the non-recursive control) is unaffected - confirmed it never even reaches the recursive-closure branch at all | nn03, nn04 |
| 3 | **A READ issued after CLOSE threw an uncaught `java.io.IOException: Stream Closed` (calling `.hasNext`/`.next()` on an iterator over an already-closed `scala.io.Source`) instead of setting FILE STATUS "47" and continuing, matching cobc's own non-crashing behavior. The refuter additionally found several wrong (non-crashing) status codes: a SECOND consecutive past-end READ should give "46" (engine repeated "10"), and re-OPENing an already-open file should give "41" (engine silently allowed it, "00"); a second CLOSE of an already-closed file should give "42"** | This generator's file-handle model had no explicit open/closed/at-end state tracking at all beyond the bare EOF path (`hasCurrentVar`) - added two new per-file flags to `fileHandleVarNames` (`file-io-gen.js`): `isOpenVar` (true from a successful OPEN until the next CLOSE) and `pastEndVar` (did the MOST RECENT read already hit end-of-file, distinguishing the first past-end READ from a second consecutive one). `generateOpen` now checks `isOpenVar` FIRST (before any handle-building codegen) and reports "41" without touching any handle when already open; `generateClose` mirrors this for "42"; `generateReadStatement` (`expression-gen.js`) checks `isOpenVar` FIRST, before ever touching `iteratorVar` - fixing the crash completely - and uses `pastEndVar` to report "46" instead of "10" for a second consecutive past-end READ. This turned out to be a FULL fix, not a partial one: all four documented status codes (41/42/46/47) are now oracle-verified correct, not left as a known gap. Verified against installed GnuCOBOL and scala-cli: nn07 byte-matches its oracle exactly (`OPEN1`/`READ1`/`READ2`/`READ3(PASTEND) STATUS=10`/`READ4(PASTEND-AGAIN) STATUS=46`/`REOPEN-WHILE-OPEN STATUS=41`/`CLOSE1`/`READ-AFTER-CLOSE STATUS=47`/`CLOSE2(ALREADY-CLOSED) STATUS=42`), no crash, program runs to completion | nn07 |
| 4 | **`PIC S9(3)PPP` (trailing P run, scale-up) and `PIC SPPP9(3)` (leading P run, scale-down) both computed identical (and wrong) scaling metadata - cobc treats them oppositely (cobc: `SCALED-UP=+123000`/`RESULT-UP=+000123000` vs engine `+0`/`+000000000`; cobc: `SCALED-DOWN=+.000000` vs engine `+0`)** | `parser/data-division-parser.js`'s PIC-scan loop `case 'P':` branch only ever branched on `inDecimalPart` (set solely by an explicit literal `V`/`.`) - always false for a plain P-only PICTURE with no V at all, so both a leading and a trailing P run fell into the identical `else` branch. Now distinguishes them via `digitCount === 0` (no digit-9 seen yet = LEADING P, before any digit - scale DOWN: each P increments `decimalDigits` and flips `inDecimalPart` true so every following digit-9 is ALSO counted as fractional, not integer) vs `digitCount > 0` (TRAILING P, after at least one digit-9 - scale UP: each P increments `integerDigits` AND decrements `decimalDigits`, driving it negative). Getting this fully correct (not just parsed, but actually producing cobc-matching runtime values) needed two small, purely-additive companion fixes this exposed, both verified as zero-risk to any pre-existing PIC shape (neither could ever have been reached before this round, since no PIC ever produced a negative `decimalDigits` or a genuinely-zero `integerDigits` paired with real `decimalDigits` info): (a) `CobolFmt.num`'s embedded runtime helper (`generateCobolFmtHelper`, `generator/expression-gen.js`) gained a dedicated `decDigits < 0` branch - render the value as a whole-number digit string zero-padded to `integerDigits`, no decimal point at all, instead of its ordinary positive-decDigits formula (which shrinks the digit string for a negative exponent instead of displaying the already-correct scaled-up magnitude); (b) `renderVariableMoveSource`'s numeric-to-numeric MOVE branch (`generator/expression-gen.js`) no longer overrides a genuinely-zero `integerDigits` with its pre-existing "18-digit unknown" fallback when the target also carries real `decimalDigits` info (a field with real PIC-derived fractional-digit metadata has a TRUSTWORTHY `integerDigits`, even when that value is 0 - distinct from a target with no PIC info at all, which still gets the 18-digit fallback unchanged). Verified against installed GnuCOBOL and scala-cli: nn09 byte-matches its oracle exactly on all 4 displayed values; confirmed zero regression on an ordinary `PIC S9(3)V99` (no P at all) probe added to `tests/round38-fixes.test.js`, and the broader suite's other PIC-parsing/numeric-MOVE tests are unaffected | nn09 |
| 5 | **Multi-target `COMPUTE`/arithmetic with `ON SIZE ERROR` treated the store decision as all-or-nothing across every target, not per target** - `COMPUTE WS-SMALL WS-BIG = 12345 + 100` (nn13: `WS-SMALL` PIC S9(1) overflows, `WS-BIG` PIC S9(5) does not) should leave only the ACTUALLY-overflowing target unchanged while storing the other's correctly computed value (cobc: `BIG=+12445` stored, `SMALL=+0` unchanged) - `generateArithmeticSizeErrorCheck` (`generator/expression-gen.js`) ORed every target's own digit-capacity overflow check into ONE combined condition and, on ANY single target's overflow, skipped storing ALL targets (its own prior doc comment's "no target is updated at all" claim was simply wrong for the multi-target case). This function is shared by ADD/SUBTRACT/MULTIPLY/DIVIDE, all of which get the same fix uniformly, not just COMPUTE | Each target's own store is now gated on ONLY that target's own `CobolFmt.fitsDigits` check (`if (CobolFmt.fitsDigits(...)) then <store>`, one such guarded line per target, inside the ON-SIZE-ERROR branch), while the ON SIZE ERROR imperative itself still fires once if ANY target's calculation overflowed (the combined OR condition governing the outer `if`/`else` is unchanged) - exactly matching COBOL's actual per-target semantics. DIVIDE's own `extraErrorCond` (BY ZERO) is a WHOLE-STATEMENT condition, not a per-target one (a zero divisor means NONE of the targets' own `resultBD` expressions can even be evaluated without raising `ArithmeticException`) - the per-target re-check is therefore additionally guarded behind `if !(extraErrorCond) then ...`, so it is never reached when the divisor is zero, preserving the pre-fix "no target touched at all" behavior for that specific (still whole-statement) condition. A single-target statement's own "OR of all targets" degenerates to just that one target's own check either way, so the single-target case (the overwhelming majority of the existing corpus) is completely unaffected - verified via a dedicated regression test in `tests/round38-fixes.test.js`. Verified against installed GnuCOBOL and scala-cli: nn13 byte-matches its oracle exactly (`SIZE-ERROR SMALL=+0 BIG=+12445`) | nn13 |

See `tests/round38-fixes.test.js` for focused, toolchain-independent unit
tests of all 5 findings above, including regression coverage confirming
nn01 (the fuller LINAGE/RECURSIVE/REDEFINES combination probe) and nn15
(the non-recursive subscripted-BY-REFERENCE control case) are unaffected,
an ordinary `PIC S9(3)V99` (no P scaling at all) computes its pre-existing,
unchanged digit metadata, and a single-target `ON SIZE ERROR` (including a
DIVIDE BY ZERO) emits the same store/no-store decision as before finding 5's
generalization.

Net effect on the whole-suite `todo` count: all 5 findings are genuine
crash-to-fix or silent-wrong-output-to-fix corrections, verified with zero
new todo entries beyond what this round's own fixes convert FROM todo/crash
TO a full, byte-exact oracle pass (nn02/nn01, nn03, nn04, nn07, nn09, nn13
all moved from failing/crashing to passing; none of the round's 9 already-
honest probes changed status). Overall dishonest-finding count for round 38:
6 (nn02, nn03, nn04, nn07, nn09, nn13 - across 5 distinct root causes, since
nn01 restates nn02's own root cause as a combination probe rather than
contributing a distinct one, while nn03/nn04 are counted separately as two
distinct operand shapes - subscripted and ref-modified - exercising the
same underlying root cause).

### Round-39 adversarial-refutation findings (oo03, oo04, oo05/oo06, oo13 x3, oo15) and their fixes

A round-39 refuter left 15 new probes (oo01-oo15) - the widest round yet, 7
dishonest findings across 7 distinct root causes. 9 probes were already
passing/honest at hand-off: oo01, oo02, oo08, oo09, oo10, oo11, oo12, oo14
byte-match their oracles cleanly; oo07 is a deliberately invalid-COBOL probe
(cobc itself rejects `PIC SPPP9(2)V9(2)` at compile time - "V cannot follow a
P which is after the decimal point" - across every P/V combination the
probe's own header comment documents having tried) - same "permanently
ineligible for `oracleCompare()`" bucket as round-34/35's jj03/kk04 and
round-7's cc04/cc07, renamed `oo07-pic-p-with-v.cbl.txt` (not `.cbl`) for the
identical reason: so `oracle.test.js`'s `walkCblFiles()` sweep (which
requires every discovered `*.cbl` to both compile AND exit zero under cobc)
doesn't hard-fail on a cobc rejection that is the entire point of the probe.
The remaining 6 probes were dishonest: oo03 (Scala compile crash in
RECURSIVE CALL BY REFERENCE codegen for a subscripted GROUP-table row), oo04
(Scala compile crash in ref-mod write codegen), oo05/oo06 (runtime
NullPointerException plus wrong FILE STATUS codes in WRITE/REWRITE/DELETE),
oo13 (three independent bugs in ACCEPT FROM DATE/DAY and a ref-mod'd
comparison operand - covers 3 of the 7 findings on its own), and oo15
(silently completing a non-RECURSIVE CALL cycle cobc itself aborts). All 7
are now fixed.

| # | Finding | Fix | Program(s) |
|---|---|---|---|
| 1 | **`CALL ... USING BY REFERENCE WS-ITEM(2)`, a subscripted element of an OCCURS table-of-GROUPS, into a RECURSIVE program crashed at Scala COMPILE time with `Not found: wsItem`** - `generateCall`'s `target.recursive` branch (`generator/expression-gen.js`)'s `isNamedGroup` check explicitly excludes any subscripted operand (correctly - a subscripted GROUP element isn't a whole-group reference), but the very next check, `isSubscriptedRefVar`, then wrongly claimed it instead - that branch assumes a single flat scalar Vector named after the group exists (`wsItem: Vector[Int]`), which it doesn't: an OCCURS-of-GROUPS table is flattened per-LEAF (`wsItemVal: Vector[Int]`, `wsItemTag: Vector[String]`), never combined into one Vector | Added a new `isSubscriptedNamedGroup` check, tested and handled BEFORE `isSubscriptedRefVar` (whose own flag now explicitly excludes this shape too): reuses `flattenGroupLeaves` (the same per-leaf traversal `isNamedGroup`'s own unsubscripted branch already uses) to build ONE getter/setter closure pair PER LEAF, each indexing that leaf's own Vector at the row subscript (`wsItemVal(1)`/`wsItemTag(1)` for `WS-ITEM(2)`) - a real, live BY REFERENCE alias for `mode === 'REFERENCE'`, or its own call-site-scoped snapshot var per leaf otherwise, mirroring `isNamedGroup`'s existing BY CONTENT/VALUE convention. Verified against installed GnuCOBOL and scala-cli: oo03 compiles and the writeback works correctly (confirmed with two additional hand-built regression probes outside the corpus - a clean, unconfounded version of the same shape with a named-variable second argument instead of oo03's own literal one - `VAL2=022`/`TAG2=ZZZ` after the call, byte-identical to a fresh, independent real-cobc capture of that same clean shape). oo03's own SPECIFIC oracle capture has one remaining single-character residual difference (`DEPTH=` blank vs `DEPTH=0`) that is NOT this finding's own root cause: independently isolated and confirmed reproducible with a minimal 2-line repro using no tables/groups/recursion at all - this exact installed GnuCOBOL build corrupts ANY bare numeric literal passed as a `CALL ... USING` argument (BY REFERENCE, BY CONTENT, and BY VALUE alike) to a blank value, a pre-existing toolchain quirk entirely orthogonal to this finding's group/subscript root cause, left as a documented, out-of-scope residual (see this round's own investigation notes) | oo03 |
| 2 | **A write through a reference-modified expression (`LK-SLICE(...) = ...`) onto a RECURSIVE program's own LINKAGE leaf crashed at Scala COMPILE time with `Reassignment to val lkSlice`** - `renderAssignment`'s `targetRef.refMod` branch (`generator/expression-gen.js`) built its `??? TODO` placeholder assignment directly with a hardcoded bare `=`, bypassing `assignExpr()` - the shared helper (added round 23) that special-cases `RECURSIVE_LEAF_NAMES.has(camel)` to emit `<camel>_=(...)` (a real setter-call) instead of assignment-sugar `=`, since Scala 3 does not desugar `x = v` into `x_=(v)` for a LOCALLY-NESTED `def`/`def _=` pair (only true object/class members support that) - every OTHER write path in this generator already routes through `assignExpr`/`renderAssignment`'s ordinary branches specifically to dodge this exact limitation; this ref-mod-write placeholder line was the one spot that didn't | Changed the hardcoded line to `assignExpr(targetCamelFor(targetRef), '??? /* TODO: ... */')`, and switched the placeholder's own comment from a line comment (`// ...`) to a block comment (`/* ... */`) - a `//` comment would otherwise swallow the wrapping `<camel>_=( ... )` call's own closing paren once routed through assignExpr's setter-call form, an unclosed-paren compile error of its own. `assignExpr` already falls back to a plain `<camel> = ...` for any name NOT in `RECURSIVE_LEAF_NAMES`, so this is a pure compile-crash fix with zero behavior change for the ordinary (ordinary module-level `var`) case - round-3's own e04 regression-guard test needed only a one-line update (line comment -> block comment in the expected pattern) to match. Verified against installed GnuCOBOL and scala-cli: oo04 compiles cleanly and runs to completion (the ref-mod write itself is still the same pre-existing, honest `???`/`NotImplementedError` decline at runtime - a known, documented gap, not something this finding was in scope to implement) | oo04 |
| 3 | **A WRITE/REWRITE/DELETE issued after CLOSE (oo05), or while a file is open in a mode that doesn't permit it (oo06: WRITE while open INPUT), dereferenced a null writer/buffer handle - a runtime NullPointerException** - round-38 finding 3 (nn07) added `isOpenVar`/`pastEndVar` lifecycle tracking, wired into `generateOpen`/`generateClose`/the READ path, but `generateWriteStatement`/`generateRewriteStatement`/`generateDeleteStatement` (`generator/expression-gen.js`) never consulted `isOpenVar` (or any access-mode flag) at all | Added a new `openModeVar` per file (`file-io-gen.js`'s `fileHandleVarNames`/`toOpenModeVarName`), a `String` holding the file's CURRENT OPEN mode ("INPUT"/"OUTPUT"/"I-O"/"EXTEND"), set alongside `isOpenVar` at both of `generateOpen`'s success points. Each of the three statement generators is now a thin wrapper (`generateWriteStatement`/`generateRewriteStatement`/`generateDeleteStatement`) around its own unchanged original body (renamed to a `...Inner` helper): `if !isOpenVar \|\| <wrong mode> then <status> else <original body>` - WRITE requires OUTPUT/I-O/EXTEND (else "48"), REWRITE/DELETE require I-O (else "49") - touching no handle at all when the guard trips, the same "check state before touching any handle" convention round-38 established for READ. Verified against installed GnuCOBOL and scala-cli: oo05 (`WRITE-AFTER-CLOSE STATUS=48`/`REWRITE-AFTER-CLOSE STATUS=49`/`DELETE-AFTER-CLOSE STATUS=49`) and oo06 (`WRITE-WHILE-INPUT STATUS=48`/`REWRITE-WHILE-INPUT STATUS=49`/`READ-STILL-WORKS STATUS=00`) both byte-match their oracles exactly; confirmed zero regression via a 20-program spot-check across the existing bb\*/cc\*/dd\* REWRITE/DELETE/WRITE corpus (all still MATCH) | oo05, oo06 |
| 4 | **`ACCEPT ... FROM DAY` silently produced the wrong value** - cobc's DAY is YYDDD (a 2-digit year plus the 3-digit day-of-year, e.g. `26201`); `generateAccept` (`generator/expression-gen.js`) rendered only `java.time.LocalDate.now.getDayOfYear.toString`, with no year component at all (`00201` once zero-padded, missing the year prefix entirely) | Prepends the 2-digit year (`java.time.LocalDate.now.getYear % 100`, zero-padded to 2 digits via an `f"..."` interpolator) to the existing 3-digit day-of-year. Verified against installed GnuCOBOL and scala-cli: oo13's own `DAY=26201` line byte-matches | oo13 |
| 5 | **`ACCEPT ... FROM DATE YYYYMMDD` (the 4-digit-year variant) was unsupported, and silently corrupted the program's own paragraph structure** - `parseAcceptStatement` (`parser/procedure-parser.js`) only ever recognized bare `DATE`/`TIME`/`DAY`/`DAY-OF-WEEK`, never checking for a trailing `YYYYMMDD` keyword extension - that token was left unconsumed and misinterpreted downstream as a new paragraph-name declaration, confirmed via direct AST inspection (a stray `UnknownStatement` node, and the DISPLAY/logic that should have followed detached from its own paragraph) - not just a wrong DISPLAYed value (`00260720` instead of `20260720`), a structural corruption that could silently break PERFORM THRU or any other paragraph-boundary-sensitive code elsewhere in ANY program using this ACCEPT variant | Two-part fix, parser AND generator (the parser side was not optional, even though the generator alone would have "fixed" the visible DISPLAY output): `parseAcceptStatement` now consumes an optional trailing `YYYYMMDD` keyword after `DATE`, setting a new `AcceptStatement.fourDigitYear` flag; `generateAccept` uses a `"yyyyMMdd"` format pattern instead of `"yyMMdd"` when that flag is set. Verified against installed GnuCOBOL and scala-cli: oo13's own `DATE4=20260720` line byte-matches, and the generated Scala was re-inspected directly to confirm exactly one `mainPara` method (no bogus second paragraph split at a phantom `YYYYMMDD` label, no stray `UnknownStatement`) - the structural corruption is gone, not just the DISPLAY output | oo13 |
| 6 | **A ref-mod'd numeric operand (`WS-DATE4(1:4)`, a ref-mod slice of a `PIC 9(8)` field) used in an IF/EVALUATE comparison crashed at RUNTIME with `NumberFormatException`** - `relationalOperandDescriptor` (`generator/expression-gen.js`) classified the operand using the BASE field's own registered (numeric) type, ignoring that COBOL reference modification ALWAYS yields an alphanumeric view regardless of the base field's declared type - this routed `renderComparisonExpr`'s numeric branch into wrapping the ref-mod read's own honest-decline empty-string placeholder in `BigDecimal(...)`, and `BigDecimal("")` throws at runtime instead of comparing safely like every other ref-mod-read honest-decline site in this codebase does | `relationalOperandDescriptor` now checks `simple.refMod` BEFORE consulting the base field's registered info, reporting `alphanumeric`/`string` for a ref-mod'd operand REGARDLESS of the base field's own declared numeric/alphanumeric type - routes the comparison through the safe string-vs-string branch instead of the crashing numeric one. Verified against installed GnuCOBOL and scala-cli: oo13's own comparison line no longer crashes (routes to the same honest-decline empty-string-vs-literal comparison every other ref-mod read already produces elsewhere in this codebase - not byte-correct, since ref-mod reads are a pre-existing, documented gap, but that was never this finding's bar) | oo13 |
| 7 | **A non-RECURSIVE program calling itself indirectly through a cycle (A calls B calls C calls A) silently ran to completion instead of matching cobc's own runtime abend** - real cobc aborts partway through the cycle with `libcob: error: recursive CALL from <X> to <Y> which is NOT RECURSIVE` (nonzero exit), producing only PARTIAL output before stopping; this engine had no notion of "this PROGRAM-ID is already active on the call stack" for a non-RECURSIVE program, so the JVM/Scala call stack just let the whole cycle run through, producing full (but spurious, since real cobc would never let this complete) output | Every non-RECURSIVE program that gets an `entry()` at all (multi-PROGRAM-ID mode) now also gets its own private `_callActive: Boolean` module-level flag (`generateEntryMethod`, `generator/scala-generator.js`) - checked at the very top of `entry()`: if already `true`, prints a cobc-style abend message and `sys.exit(1)`; otherwise sets it `true` and runs the program's own body inside `try ... finally { _callActive = false }`. `try/finally` (not a bare sequential reset statement after the body) specifically because GOBACK/EXIT PROGRAM compiles to a bare Scala `return` - a non-local return that would otherwise skip straight past a trailing reset line, leaving the flag stuck `true` forever and wrongly abending every LATER, legitimate, non-overlapping sequential call to the same program; `finally` unwinds through a `return` correctly regardless. A genuinely RECURSIVE program (or one that falls through to this same code path despite being declared RECURSIVE, because its own LINKAGE shape can't be flattened) gets NO guard at all - gated on `!isRecursiveProgram(ast)` both at the `_callActive` var's own declaration site and at the guard-emission site inside `generateEntryMethod`, since it legitimately re-enters via `generateRecursiveEntryMethod`'s own separate per-activation closure mechanism. Verified against installed GnuCOBOL and scala-cli: oo15's generated Scala, run directly, prints exactly `MAIN CALLING A`/`IN A N=1`/`IN B N=1`/`IN C N=1`/`libcob: error: recursive CALL into OO15PA which is NOT RECURSIVE` and exits 1 - byte-identical to the real cobc capture (down to which lines print before the abend); confirmed zero regression on j10/dd02/k01 (genuinely RECURSIVE programs, no guard at all) and v01/d13/j09 (a non-RECURSIVE subprogram called repeatedly in sequence, never overlapping - still MATCH, the guard correctly resets after each call) | oo15 |

See `tests/round39-fixes.test.js` for focused, toolchain-independent unit
tests of all 7 findings above, including regression coverage confirming a
RECURSIVE program's own writeback/entry-method conventions (nn03's plain
subscripted-scalar shape, e04's ordinary non-RECURSIVE-leaf ref-mod write,
j10's genuine recursion, d13's repeated sequential non-RECURSIVE calls) are
all unaffected.

oo15 is a special case for the AUTOMATED oracle-corpus suite specifically:
`oracleCompare()`'s own contract requires the real cobc side to exit 0 to
ever report a `match` (real cobc for this probe deliberately exits 1 - that
IS the point of the probe), so `oracle.test.js`'s Phase 2 describe block
will always register oo15 as a `t.todo(...)` ("COBOL side did not run
cleanly") no matter how correct the fix is - this is a structural property of
the harness, not a residual bug; renaming to `oo15-nonrecursive-call-cycle.cbl.txt`
(matching oo07/jj03/kk04/cc04/cc07's own established precedent for a program
whose own correct cobc behavior is a nonzero exit or compile rejection) keeps
it out of the "cobc oracle capture" sweep's hard `exitCode === 0` assertion
entirely, while `tests/round39-fixes.test.js` and this round's own manual
`scala-cli run` verification (see finding 7's own write-up above) independently
confirm the fix is byte-correct end-to-end. oo07 is excluded from the sweep
for the identical reason (a deliberately invalid-COBOL probe, cobc itself
