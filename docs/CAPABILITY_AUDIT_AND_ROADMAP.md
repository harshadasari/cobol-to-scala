...
| Batch/sort | SORT (multi-key, mixed ASCENDING/DESCENDING, `THRU` ranges) via INPUT/OUTPUT PROCEDURE, RELEASE, RETURN — the whole-file `SORT ... USING <file> GIVING <file>` form (no procedure) is not yet implemented (visible TODO marker). MERGE `... USING file1 file2 ... OUTPUT PROCEDURE` (multi-key, mixed ASCENDING/DESCENDING) is now genuinely implemented and oracle-verified as of round 18 (`tests/corpus/proc/g12-merge-outputproc.cbl`) — **this row's earlier claim that MERGE was already oracle-equivalent was false**: `generateExpression`'s statement-type switch had no `'MERGE'` case at all before round 18 (a `MergeStatement` node silently fell through to a no-op), so no MERGE statement of any shape actually ran; see `tests/oracle/README.md`'s round-18 table. MERGE's own `GIVING` form (no OUTPUT PROCEDURE) remains an unimplemented visible TODO, same as SORT's |
| File I/O | OPEN, CLOSE, READ, WRITE for LINE SEQUENTIAL files are now oracle-equivalent (round-5 fix: `convertToScala()`'s own entry point never parsed the ENVIRONMENT DIVISION at all before this — FILE-CONTROL/SELECT...ASSIGN never reached the generator, so every OPEN/WRITE/READ silently referenced undeclared variables; see `tests/corpus/proc/s01-fileio-roundtrip.cbl`). `OPEN I-O` (round 25: previously never initialized the read iterator at all, so any READ after it always reported FILE STATUS 10 regardless of what was on disk — `tests/corpus/proc/o03-io-mode-read.cbl`) and REWRITE/DELETE (round 25: previously bare `// REWRITE ...`/`// DELETE ...` COMMENT no-ops with no runtime effect and no visible marker — `tests/corpus/proc/o01-rewrite-field.cbl`, `o02-delete-record.cbl`) are now genuinely implemented and oracle-verified for a SEQUENTIAL-access RELATIVE/INDEXED file: OPEN I-O loads the file's records into an in-memory, position-tracked line buffer (the same "one text line per record" storage model READ/WRITE already use — this generator's only modeled file organization, never true byte-random access), REWRITE/DELETE mutate that buffer in place at the current read position, and CLOSE flushes it back to disk. START and RANDOM/DYNAMIC access by an explicit RELATIVE/RECORD KEY (as opposed to "whatever was just READ") remain unimplemented comment-only stubs |
...
- **What "verified" does not mean:** it does not mean every COBOL construct
  ...
  TODO markers, START and RANDOM/DYNAMIC key-based access remain unimplemented
  stubs (REWRITE/DELETE were fixed for real, SEQUENTIAL-access semantics in
  round 25 — see `tests/oracle/README.md`), and two
