...
| COMPUTE/ADD/SUBTRACT/MULTIPLY/DIVIDE truncation semantics without ROUNDED | **OPEN, newly identified while preparing this document** — arithmetic assignment never truncates to the target field's declared decimal digits at assignment time; only `DISPLAY`-time formatting (`CobolFmt.num`, always `HALF_UP`) fixes the digit count. Verified directly against `cobc`: `COMPUTE X = 2.345` into a 2-decimal COMP-3 field with no ROUNDED clause should truncate to `2.34` (real GnuCOBOL output); the generated Scala instead prints `2.35` (same as the ROUNDED case) because the stored value is never truncated before display. Not caught by the existing 48-program corpus because no corpus case happens to have a truncation-vs-rounding-discriminating final digit. This is a real, reproducible divergence, not a hypothetical — reproduction kept in `demo/README.md`'s caveats section rather than as a new corpus program, since this pass is docs/demo-only and does not touch `generator/`. | Medium/High — silently wrong money math is exactly the class of bug byte-level verification exists to catch; this one slipped through because the corpus didn't exercise the discriminating case, not because the harness is unsound |
